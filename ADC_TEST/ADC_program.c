...
u8 ADC_u8StartConversionAsynch(u8 Copy_u8Channel,u16* Copy_pu16Result, void (*Copy_pvNotificationFunc)(void))
{
	...
	{
		Local_u8ErrosState= NULL_POINTER;
	}
...
u8 ADC_u8StartChainConversionASynch(u8 Copy_u8NumberOfConversions,u16 Copy_pu16Result[],u8 Copy_puNeededChannels[], void(*Copy_pvNotificationFunction)(void)) {
	...
	if( (Copy_pu16Result != NULL) && (Copy_pvNotificationFunction != NULL) && (Copy_puNeededChannels != NULL) ) {
		...
		ADC_pu8NeededChannels = Copy_puNeededChannels;

		//Clear first 5 bits  in ADMUX register
		ADMUX &= ADC_u8CHANNEL_MASK;

		/*Set required channel*/
		ADMUX |= Copy_puNeededChannels[ADC_u8Counter];
