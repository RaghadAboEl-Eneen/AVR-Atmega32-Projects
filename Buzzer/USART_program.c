...
#error "Wrong double transmission speed configuration"
	...
	Local_u8UBBRHValue = Local_u16UBBRValue >> 8;

	//Assigning the UBRRH value in one operation
	//UBRRH = Local_u8UBBRHValue;
	//UBRRL = (u8) Local_u16UBBRValue;
...
void USART_voidSendNumberSynchronous(u32 Copy_u32Number) {
	...
	USART_u8SendStringSynchronous(arr);
...
void __vector_13(void) {
	...
	if ((Local_u8ReceivedData >= 48 && Local_u8ReceivedData <= 57) || (Local_u8ReceivedData >= 65 && Local_u8ReceivedData <= 90) || (Local_u8ReceivedData >= 97 && Local_u8ReceivedData <= 122)) {

		USART_pu8ReceivedData[USART_u16Counter] = Local_u8ReceivedData;
